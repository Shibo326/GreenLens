# GreenLens AI ΓÇö Greenwashing Detection Powered by AMD MI300X

> Upload sustainability reports and marketing materials ΓåÆ AI cross-references claims against data and flags contradictions, vague language, and unverified assertions in under 90 seconds.

**Live Demo:** https://amd-hackthon-ll14.vercel.app  
**Backend API:** https://amdhackthon-production.up.railway.app  
**GitHub:** https://github.com/Shibo326/GreenLens  
**Track:** AI for Sustainability ≡ƒî▒ ΓÇö YFS Build for Good Hackathon
...
- ≡ƒöì Detects **greenwashing contradictions** between what companies claim and what their own data shows
- ≡ƒôè Generates a **Greenwash Score (0ΓÇô100)** ΓÇö color-coded credibility gauge
- ΓÜû∩╕Å Builds a **Claim vs. Reality Matrix** comparing marketing language against reported data
- ΓÜí **Quick Scan** ΓÇö instant verdict on any sustainability claim, no documents needed
- ≡ƒô╖ **Snap & Check** ΓÇö photograph product labels/packaging for AI vision analysis
- ≡ƒÆ¼ **Chat Copilot** ΓÇö sustainability analyst persona with RAG-grounded citations
- ≡ƒºÆ **ELI15 Mode** ΓÇö toggle to simplify AI responses for younger audiences
- ≡ƒîÉ **Bilingual** ΓÇö responds in English and Filipino/Tagalog (language matching)
- ≡ƒôä Exports full analysis as **PDF or DOCX**
...
Γöé  Frontend (React 18 / Vite / TailwindCSS 4)  ΓåÆ Vercel  Γöé
...
Γöé  Backend (FastAPI / Python 3.11)             ΓåÆ Railway  Γöé
Γöé  Γö£ΓöÇΓöÇ RAG: ChromaDB + all-MiniLM-L6-v2 embeddings       Γöé
Γöé  Γö£ΓöÇΓöÇ 5 parallel LLM calls via asyncio.gather            Γöé
Γöé  ΓööΓöÇΓöÇ Semaphore(3) rate-limit protection                 Γöé
ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ
                         Γöé HTTPS
ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓû╝ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ
Γöé  Fireworks AI  (AMD Instinct MI300X hardware)           Γöé
Γöé  Γö£ΓöÇΓöÇ deepseek-v4-flash  (quality tier ΓÇö reasoning)     Γöé
Γöé  ΓööΓöÇΓöÇ gpt-oss-120b       (fast tier ΓÇö 646 tok/s)        Γöé
...
- Python 3.11+
- Node.js 18+
- Fireworks AI API key ΓåÆ https://app.fireworks.ai
...
cd backend
cp .env.example .env
# Edit .env ΓÇö add your FIREWORKS_API_KEY
pip install -r requirements.txt
python main.py
# ΓåÆ http://localhost:8000
```
...
cd frontend
cp .env.example .env
# Edit .env ΓÇö set VITE_API_URL=http://localhost:8000
npm install
npm run dev
# ΓåÆ http://localhost:5173
```
...
|----------|----------|---------|-------------|
| `FIREWORKS_API_KEY` | Γ£à | ΓÇö | Fireworks AI API key |
| `FIREWORKS_ENDPOINT` | Γ£à | `https://api.fireworks.ai/inference/v1` | Fireworks API base URL |
| `FIREWORKS_MODEL_QUALITY` | ΓÇö | `accounts/fireworks/models/deepseek-v4-flash` | Quality tier model (reasoning) |
| `FIREWORKS_MODEL_FAST` | ΓÇö | `accounts/fireworks/models/gpt-oss-120b` | Fast tier model (MI300X optimized) |
| `FIREWORKS_MODEL_VISION` | ΓÇö | *(disabled)* | Vision model for Snap & Check |
| `SINGLE_CALL_MODE` | ΓÇö | `false` | Emergency: combine all analysis into 1 call |
| `ALLOWED_ORIGINS` | ΓÇö | `*` (all) | Comma-separated CORS origins |
| `PORT` | ΓÇö | `8000` | Server port |

### Frontend (`frontend/.env`)

| Variable | Required | Description |
|----------|----------|-------------|
| `VITE_API_URL` | Γ£à | Backend URL (e.g., `http://localhost:8000` or Railway URL) |
...
|--------|------|-------------|
| `POST` | `/api/upload` | Upload documents (PDF, PNG, JPEG, DOCX) ΓÇö max 10 files, 10 MB each |
...
| `POST` | `/api/chat/vision` | Snap & Check ΓÇö analyze product label/packaging image |
...
### Why AMD MI300X?
`gpt-oss-120b` on Fireworks AI runs on AMD Instinct MI300X at **646 tokens/sec** ΓÇö enabling the fast-tier structured extraction calls (comparison matrix, conflict detection) to complete in 2ΓÇô4 seconds each. This makes the 5-parallel-call architecture viable within a 90-second budget.
...
| `/dashboard` | Dashboard | Upload ΓåÆ Analyze ΓåÆ Results (Score, Risks, Matrix, Conflicts) |
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| "Analysis failed" immediately | Wrong model ID in Railway | Check `FIREWORKS_MODEL_QUALITY` ΓÇö no spaces or `=` in value |
| "Connection error" on frontend | `VITE_API_URL` misconfigured | Must be exact URL, no trailing slash. Redeploy after change |
| 0 risks / empty matrix | `max_tokens` too low | Check Railway logs ΓÇö model may be truncating |
