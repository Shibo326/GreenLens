...
Endpoints:
  POST /api/prompts/versions  -- register a new prompt version
  POST /api/prompts/evaluate  -- evaluate a candidate prompt against production
  POST /api/prompts/promote   -- promote a version to production
  POST /api/prompts/rollback  -- roll back to the previous production version
"""
...
async def register_prompt_version(body: RegisterVersionRequest):
    """
    Register a new prompt version with name, content, and semantic version string.
    """
    ...
    try:
        ...
        return JSONResponse(
            status_code=500,
            content={"error": "Failed to register prompt version.", "code": "INTERNAL_ERROR"},
        )
...
async def evaluate_prompt(body: EvaluateCandidateRequest):
    """
    Evaluate a candidate prompt version against the current production version.
    ...
    try:
        ...
        return JSONResponse(
            status_code=500,
            content={"error": f"Evaluation failed: {str(e)[:200]}", "code": "EVALUATION_ERROR"},
        )
...
async def promote_prompt(body: PromoteRequest):
    ...
    Succeeds only if the version beats the current production on all categories.
    """
    svc_err = _check_services()
    if svc_err:
        return svc_err
    try:
        success = await _prompt_optimizer.promote(
            prompt_name=body.prompt_name,
            version=body.version,
        )
        if success:
            ...
            return JSONResponse(
                status_code=400,
                content={
                    "error": "Promotion failed: candidate does not beat production on all categories.",
        ...
        return JSONResponse(
            status_code=500,
            content={"error": "Failed to promote prompt.", "code": "INTERNAL_ERROR"},
        )
...
async def rollback_prompt(body: RollbackRequest):
    """
    Roll back a prompt to its previous production version.
    """
    svc_err = _check_services()
    if svc_err:
        return svc_err
    try:
        ...
        return JSONResponse(
            content={"status": "rolled_back", "active_version": previous_version}
        )
    except ValueError as e:
        return JSONResponse(
            status_code=422,
        ...
        return JSONResponse(
            status_code=500,
            content={"error": "Failed to rollback prompt.", "code": "INTERNAL_ERROR"},
        )